...
mxmlc -debug -swf-version=27 -incremental=false -output src/Main.swf "-compiler.source-path+=locale/{locale}" +configname=airmobile -source-path+=src -locale=en_US -locale=fr_FR -library-path+=FB-libs "-library-path+=locale\{locale}\fiber_rb.swc" -library-path+="..\dns-flex\target\dns.swc" -- src/Main.mxml
